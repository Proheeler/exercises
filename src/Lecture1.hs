...
lowerAndGreater n list = show n ++
  ...
  ++ show greater
  ++  " elements"
  where countGreatestAndLowes :: [Int] -> (Int, Int) -> (Int, Int)
        countGreatestAndLowes [] tuple = tuple
        countGreatestAndLowes (x:xs) tuple
          | x == n = countGreatestAndLowes xs tuple
          | n > x = countGreatestAndLowes xs (fst tuple, snd tuple + 1)
          | otherwise = countGreatestAndLowes xs (fst tuple + 1, snd tuple)
        result = countGreatestAndLowes list (0, 0)
        greater = fst result
        lower = snd result
